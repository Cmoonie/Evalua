...
@section('content')
    <div class="container mx-auto p-6">
        <h1 class="text-3xl font-bold mb-6">Formulier bewerken: {{ $forms->firstWhere('id', $filledForm->form_id)->title }}</h1>
        ...
        <form action="{{ route('filled_forms.update', $filledForm) }}" method="POST">
            ...
            @foreach($forms->firstWhere('id', $filledForm->form_id)->formCompetencies as $formCompetency)
                                                ...
                                                @foreach($component->levels as $level)
                                                    @if(strtolower($level->gradeLevel->name) === $grade)
                                                        <button type="button"
                                                                ...
                                                                {{ $grade=='goed'?'bg-green-200 border-green-400'
                                                                :($grade=='voldoende'
